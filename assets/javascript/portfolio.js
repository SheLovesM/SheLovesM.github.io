...
const ipgeolocation = 'https://api.ipgeolocation.io/ipgeo?apiKey=fcbda2cf85e74eda8e62e67b2f5d352a';
...
$(document).ready(() => {
  ...
  app.titleChanger(['skidded.systems', 'skid on top', 'professional skid', 'best shittalker']);
...
$.getJSON(ipgeolocation, (data) => {
  writeLine(['Authenticating...', "Granting access to <span style='font-size: 14px; color: #06d;'>[skidded.systems]</span>..."], 30, () => {
    ...
    writeLine([`Access granted! <span style='font-size: 14px; color: #0f0;'>[success]</span>`, `Welcome back, <i style='color: #0f0'>${ip}</i>! By the way, nice to see someone from <i style='color: #0f0'>${country}</i> here!`], 30, 500, () => {
      ...
      writeLine([`<i style='color: #F62459'>cool $$$</i>`], 120, 500, () => {
        timeouts.push(
          setTimeout(() => {
            if (app.skippedIntro) return;
